...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withReads; query != nil {
		...
		for _, n := range neighbors {
			fk := n.user_reads
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_reads" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_reads" returned %v for node %v`, *fk, n.ID)
			}
